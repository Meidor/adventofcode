...
impl BitsPacket {
    ...
    fn from_bitslice(bits: &BitSlice<u8, Msb0>) -> BitsPacket {
        let mut pp: usize = 0;
        ...
        if operator == OperatorType::Literal {
            ...
            let mut vbits: BitVec<u8, Msb0> = bitvec![u8, Msb0; 0];
            while !is_last {
                is_last = !bits[pp];
    ...
    fn get_string(slice: &BitSlice<u8, Msb0>) -> String {
    ...
    fn parse_u64(bits: &BitSlice<u8, Msb0>) -> u64 {
        u64::from_str_radix(&BitsPacket::get_string(bits), 2).unwrap()
    }

    fn parse_u8(bits: &BitSlice<u8, Msb0>) -> u8 {
        u8::from_str_radix(&BitsPacket::get_string(bits), 2).unwrap()
    }

    fn parse_operator(bits: &BitSlice<u8, Msb0>) -> OperatorType {
