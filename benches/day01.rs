...
fn bench_part_one_old_school(c: &mut Criterion) {
    let lines = read_lines(std::path::Path::new("./inputs/day01.txt")).unwrap();
    c.bench_function("day01_part_1_old_school", |b| {
        b.iter(|| day01::part_one_old_school(black_box(&lines)))
    });
}

fn bench_part_two_old_school(c: &mut Criterion) {
    let lines = read_lines(std::path::Path::new("./inputs/day01.txt")).unwrap();
    c.bench_function("day01_part_2_old_school", |b| {
        b.iter(|| day01::part_two_old_school(black_box(&lines)))
    });
}
